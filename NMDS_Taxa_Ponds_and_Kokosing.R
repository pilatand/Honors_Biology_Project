...
Relevant_Metadata_2021 <- Data2021[,c(4:6,8:11)]
...
Interest <- env.scores.2021[c(1, 4),]
noInterest <- env.scores.2021[c(2,3, 5:7),]
...
master2021 <- ggplot(site.scrs, aes(x=NMDS1, y=NMDS2))+ #sets up the plot
  ...
  scale_y_continuous(labels = scales::number_format(accuracy = 0.1))+geom_text_repel(data = site.scrs, size =0, fontface="bold",
                                                                                     aes(x = NMDS1, y = NMDS2, label = genera), color="black", nudge_y = -0.02)+
  geom_rect(aes(xmin=0.04, xmax = 0.18, ymin = -0.32, ymax = -0.15), fill = NA, color = "black")+
  geom_segment(aes(x=0.04, y= -0.15, xend = 0.6, yend = -0.68), color = "black", linetype = "dashed")+
...
inset_plot <- ggplot(site.scrs, aes(x=NMDS1, y=NMDS2))+ #sets up the plot
  geom_point(aes(x=NMDS1, y=NMDS2), color = "black")+ #adds site points to plot, shape determined by Landuse, colour determined by Management
  coord_cartesian(xlim=c(0.05, 0.17), ylim=c(-0.3, -0.18))+
  geom_text_repel(data = site.scrs, size =0, fontface="bold",
...
plot_with_inset <- master2021 +
  annotation_custom(
    grob = inset_grob,
    xmin = 0.5, xmax = 1.4,  # Adjust these values based on your main plot's limits
    ymin = -1.5, ymax = -0.6     # Adjust these values based on your main plot's limits
  )

tiff("2021 NMDS Community Inlet", units = "in", width =10, height=9, res=300)
print(plot_with_inset)
dev.off()

##2022 Pond Community Structure##
Data2021 <- read.csv("/Users/andrewpilat/Documents/Honors/Data/Data2022.csv")
Data2022 <- Data2022[1:77,]
Class_2022_data <- Data2022[,32:56]
Relevant_Metadata_2022 <- Data2022[,c(4:6,8:11)]
...
##Fit the environmnetal variables
MDS_2022_Env_Fit <- envfit(MDS_2022, Relevant_Metadata_2022, permutations = 999)
head(MDS_2022_Env_Fit)


##Dataframe for ggplot. This one pulls the NMDS data
site.scrs.2022 <- as.data.frame(scores(MDS_2022, display = "species")) #save NMDS results into dataframe
...
Interest <- env.scores.2022[c(1, 4),]
noInterest <- env.scores.2022[c(2,3, 5:7),]
...
master2022 <- ggplot(site.scrs.2022, aes(x=NMDS1, y=NMDS2))+scale_y_continuous(labels = scales::number_format(accuracy = 0.1))+ #sets up the plot
  geom_point(aes(x=NMDS1, y=NMDS2), color = "black")+ #adds site points to plot, shape determined by Landuse, colour determined by Management
  coord_cartesian(xlim=c(-1.5,1.5), ylim=c(-1.5,1.5))+scale_x_continuous(labels = scales::number_format(accuracy = 0.1))+
  geom_text_repel(data = site.scrs.2022, size =0,fontface="bold", nudge_y = 0.01,
                  aes(x = NMDS1, y = NMDS2, label = genera), color="black")+
  geom_rect(aes(xmin=-0.32, xmax = 0.12, ymin = -0.3, ymax = -0.05), fill = NA, color = "black")+
...
inset_plot <- ggplot(site.scrs.2022, aes(x=NMDS1, y=NMDS2))+ #sets up the plot
  geom_point(aes(x=NMDS1, y=NMDS2), color = "black")+ #adds site points to plot, shape determined by Landuse, colour determined by Management
  coord_cartesian(xlim=c(-0.3, 0.1), ylim=c(-0.27, -0.06))+
  geom_text_repel(data = site.scrs.2022, size =0, fontface="bold",
                  aes(x = NMDS1, y = NMDS2, label = genera), color="black")+
  theme_classic()+ 
  theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = "solid"))+
  theme(legend.position = "none", axis.title = element_blank(), axis.text = element_blank(), panel.grid = element_blank(),
...
tiff("2022 NMDS Community Inlet", units = "in", width =10, height=9, res=300)
print(plot_with_inset)
dev.off()

####Kokosing River####

Kokosing_Complete <- read.csv("/Users/andrewpilat/Documents/Honors/Data/Kokosing_Data.csv")
Kokosing_Complete <- Kokosing_Complete[1:18,]
rownames(Kokosing_Complete) <- Kokosing_Complete$Table.for.all.samples

Kokosing_Metadata <- Kokosing_Complete[,3:9]
Kokosing_Taxa <- Kokosing_Complete[,10:35]
